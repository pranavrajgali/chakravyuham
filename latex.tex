...
\author{Abhinav \and Pranav Raj \and Srinidhi \and Tejasri \and Darshitha}
\date{}
...
\subsubsection{2. Message-Count-Based Windowing}
\begin{itemize}[noitemsep]
    \item Continuous CAN traffic streams are segmented using sliding message-count windows of size $N=100$ and step $S=50$.
    \item Each window captures a stable, fixed-size sequence of vehicle communication.
    \item Message-count-based windowing was selected over fixed-time windowing because it guarantees a constant number of packets per window. This consistency is mathematically necessary to compute stable statistical metrics—such as variance, entropy, and Hamming distances—preventing division-by-zero or numerical instability when the vehicle is idling or stationary and transmission frequency drops.
...
Instead of relying on raw CAN identifiers, Chakravyuham represents every communication window using a Unified Standard Feature Vector ($V_W$) computed over sliding message-count windows:

$$V_W = [ \mu_{IAT}, \sigma^2_{IAT}, F_{window}, \mu_{Entropy}, \sigma^2_{Entropy}, \mu_{Hamming}, U_{ID\_Ratio} ]$$

The feature vector encapsulates the following engineered behavioural signals:

\paragraph{Timing \& Frequency Behaviour}
\begin{itemize}[noitemsep]
    \item \textbf{Inter-Arrival Time (IAT) Mean ($\mu_{IAT}$)}: Measures the average time elapsed between consecutive transmissions grouped by ID, identifying volumetric anomalies such as DoS flooding.
    \item \textbf{Jitter (IAT Variance $\sigma^2_{IAT}$)}: Measures variation in inter-arrival times grouped by ID, detecting clock drift and timing anomalies induced by spoofing.
    \item \textbf{Message Frequency ($F_{window}$)}: Computed as $F_{window} = \frac{N_W}{W}$ bus-wide, detecting overall transmission density spikes.
    \item \textbf{Unique ID Ratio ($U_{ID\_Ratio}$)}: Defined as $U_{ID\_Ratio} = \frac{\text{Unique IDs}}{N_W}$. This ratio drops to $\approx 0$ during DoS flooding attacks (as one ID dominates the window) and spikes to $\approx 1$ during random Fuzzing attacks.
\end{itemize}

\paragraph{Payload \& Content Behaviour}
\begin{itemize}[noitemsep]
    \item \textbf{Payload Shannon Entropy ($\mu_{Entropy}$, $\sigma^2_{Entropy}$)}: Computed as $H(X) = - \sum p_i \log_2 p_i$. Quantifies byte randomness; normal telemetry exhibits low entropy, while Fuzzy injection payloads exhibit high entropy.
    \item \textbf{Payload Hamming Distance ($\mu_{Hamming}$)}: Measures the bitwise differences between consecutive payloads of the same ID, capturing anomalous physical state transitions (like fake RPM or speed spikes) independent of raw values.
\end{itemize}

\paragraph{Structural \& Graph Behaviour (GCN Pathway)}
\begin{itemize}[noitemsep]
    \item \textbf{Edge Weights \& Directed Transitions}: Capture sequence transition statistics between ECUs.
    \item \textbf{Graph Topology Metrics}: Describe the structural connectivity of the communication graph to detect topological anomalies.
\end{itemize}
...
\subsubsection{6. Image-Based Pipeline (MosaicCNN)}
To establish a baseline for spatial-temporal representation learning directly on payloads, we implement a Convolutional Neural Network operating on raw CAN sequence images (MosaicCNN):
\begin{itemize}[noitemsep]
    \item \textbf{Image Construction (Mosaic Grid)}: The 29-bit CAN IDs (optionally compressed via a trained Autoencoder to 9 bits) are combined with byte payloads to form a $32 \times 32$ binary matrix window (the "Mosaic").
    \item \textbf{Spatial Convolution}: The binary grid is fed into a 2D CNN with stacked Conv2D, MaxPool2D, and fully connected layers.
    \item \textbf{Comparative Role}: MosaicCNN operates as a spatial pattern baseline, mapping high-frequency payload and ID transitions visually to detect injection anomalies.
\end{itemize}
...
\subsection{Candidate Model Architectures Summary}
To provide a rigorous architectural comparison, we benchmark five different models ranging from traditional statistical classifiers to deep neural network pathways. Table~\ref{tab:models_summary} outlines their architectural configurations, serialized parameters, and theoretical roles in the defense ecosystem.

\begin{table}[htbp]
\centering
\caption{Comparative Summary of Candidate Model Architectures}
\label{tab:models_summary}
\begin{tabular}{llccl}
\toprule
\textbf{Model Class} & \textbf{Algorithm} & \textbf{Memory Size} & \textbf{Avg Latency} & \textbf{Core Structural Role} \\ \midrule
...
Image CNN & MosaicCNN & 0.74 MB & 1.52 ms & 2D spatial convolution on binary packet grids \\ \bottomrule
\end{tabular}
\end{table}
...
\subsection{Edge Deployment and Lightweight Inference Pipeline}
Automotive in-vehicle systems operate under strict real-time execution constraints and extremely limited memory limits. To demonstrate the real-world deployability of the Chakravyuham detection system on Electronic Control Units (ECUs), we benchmarked the models on constrained edge configurations:
\begin{itemize}[noitemsep]
    \item \textbf{LightGBM}: Serialized model size of \textbf{1.34 MB} (reducing to \textbf{0.68 MB - 0.85 MB} under downsampled configurations) with an average inference latency of \textbf{0.92 ms - 1.18 ms} per window.
    \item \textbf{XGBoost}: Serialized model size of \textbf{0.95 MB} (reducing to \textbf{0.38 MB - 0.69 MB} under downsampled configurations) with a sub-millisecond inference latency of \textbf{0.38 ms}.
    \item \textbf{GCNIDS}: Serialized weights size of \textbf{0.0033 MB} (extremely compact) with an average inference latency of \textbf{10.3 ms} (constrained by graph-construction overhead on low-cost hardware).
    \item \textbf{MosaicCNN}: Serialized size of \textbf{0.74 MB} with a latency of \textbf{1.52 ms}.
\end{itemize}

\textbf{Proposed Solution}
\begin{itemize}[noitemsep]
    \item Tree models compiled directly to optimized C code using compiled libraries (Treelite / m2cgen) to eliminate interpreter and runtime overhead.
    \item Bounded-memory rolling circular arrays for real-time windowing and z-score baseline updates.
    \item Real-time inference optimization on embedded microcontrollers.
\end{itemize}

\textbf{Prototype Scope}
\begin{itemize}[noitemsep]
    \item Raspberry Pi 4 and NVIDIA Jetson Nano edge benchmarking.
    \item Validation of CPU usage, execution jitter, and heap allocations under continuous CAN traffic simulation.
\end{itemize}

\textbf{Objective}
Ensure \textbf{real-time feasibility under embedded ECU constraints}
...
\section{Tech stack}
To facilitate rapid prototyping, validation, and dashboard visualization, we selected an enterprise-grade technology stack outlined in Table~\ref{tab:tech_stack}.

\begin{table}[htbp]
\centering
\caption{Chakravyuham Development and Operations Technology Stack}
\label{tab:tech_stack}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lll}
\toprule
\textbf{Category} & \textbf{Tooling / Libraries} & \textbf{Implementation Role} \\ \midrule
\textbf{Programming} & Python, TypeScript, Node.js, Vite & Core backend research, FastAPI service, and React 19 dashboard \\
\textbf{Data Handling} & Pandas, NumPy & Ingestion of raw logs, rolling statistics, and feature normalization \\
\textbf{Machine Learning} & Scikit-learn, XGBoost, LightGBM & Statistical GBDT classification and latency evaluation \\
\textbf{Graph Networks} & PyTorch, PyTorch Geometric (PyG) & 2D GCN spatial convolution and topological routing models \\
\textbf{Explainability} & SHAP (TreeSHAP) & Anomaly contribution analysis and shortcut memorization checks \\
\textbf{Edge Deployment} & ONNX, Treelite, m2cgen & Lightweight serialization and tree-to-C compilation targets \\
\textbf{Dashboard} & FastAPI, Tailwind CSS, Framer Motion & Live dashboard log monitoring, charts, and telemetry averages \\
\textbf{Datasets} & HCRL (Car-Hacking), OTIDS (KIA Soul) & Raw in-vehicle attack captures and cross-validation logs \\ \bottomrule
\end{tabular}%
\end{table}
